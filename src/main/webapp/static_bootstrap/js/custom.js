...
function charts() {
	...
	if($("#increaseUser").length)
	{
		...
		var previousPoint = null;
		$("#increaseUser").bind("plothover", function (event, pos, item) {
			$("#x").text(pos.x.toFixed(2));
			$("#y").text(pos.y.toFixed(2));
		...
		$("#increaseUser").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
				plot.highlight(item.series, item.datapoint);
			}
	...
	if($("#liveUser").length)
	{
		...
		var previousPoint = null;
		$("#liveUser").bind("plothover", function (event, pos, item) {
			$("#x").text(pos.x.toFixed(2));
			$("#y").text(pos.y.toFixed(2));
		...
		$("#liveUser").bind("plotclick", function (event, pos, item) {
			if (item) {
				$("#clickdata").text("You clicked point " + item.dataIndex + " in " + item.series.label + ".");
				plot.highlight(item.series, item.datapoint);
			}
	...
	if($("#macChart").length)
	{
		...
		$("#macChart").bind("plothover", pieHover);
	}
	...
	if($("#networkChart").length)
	{
		...
		$("#networkChart").bind("plothover", pieHover);
	}
