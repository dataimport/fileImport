...
<#macro select
	...
	id="" name="" isSearchIndex="" class="" style="" size="" title="" disabled="" tabindex="" accesskey=""
	vld=""
	...
	<#if listKey!="" && listValue!="">
		...
		<#list list as item>
		  <#if isSearchIndex!="">
			...
			<#else>
				<option  disabled ="true"  value="${item[listKey]}"<#if item[listKey]?string==value?string> selected="selected"</#if>><#if listDeep!="" && item[listDeep] gte origDeep><#list origDeep..item[listDeep] as i>&nbsp;&nbsp;</#list>></#if>${item[listValue]!}</option><#t/>
			</#if>
		  <#else>
			<option  value="${item[listKey]}"<#if item[listKey]?string==value?string> selected="selected"</#if>><#if listDeep!="" && item[listDeep] gte origDeep><#list origDeep..item[listDeep] as i>&nbsp;&nbsp;</#list>></#if>${item[listValue]!}</option><#t/>
		  </#if>
