...
<table width="100%" border="0" cellpadding="0" cellspacing="0"><tr><td align="center" class="pn-sp">
	...
	当前 ${value.pageNo}/${value.totalPage} 页 &nbsp;转到第<input type="text" id="_goPs" style="width:50px" onkeyup="value=value.replace(/[^\d]/g,'')"  onfocus="this.select();" onkeyup="value=value.replace(/[^\d]/g,'') " ; onkeypress="if(event.keyCode==13){$('#_goPage').click();return false;}"/>页
...
function _gotoPage(pageNo) {
	try{
	    if(pageNo>${value.totalPage}) pageNo = ${value.totalPage};	   
